...
extern string  comment3 = "==== Drivers options =====";
...
void _positions_open ()
   ...
   // 1. BUY ORDERS
   // 1.1. FIND WAVE BUY
   if (iBarShift (Symbol(), timeframe_Lowest, lastTime, false) > 0 || lastTime == 0){
      lastTime = TimeCurrent ();
//      if (deal_costs_max_koef>0) found_wave_BUY = wave.FindAll (OP_BUY, Point * costs_pips / deal_costs_max_koef);
       if (deal_costs_max_koef>0) found_wave_BUY = wave.FindCascade(OP_BUY,0.00300, 1440, 1);  
...
void delete_f_number (int fNumberLocal, int directionLocal){
   ...
   if (!ObjectDelete (objName)) log_add_line ("Ошибка удаления цифры "+fNumberLocal,Red);
   }
